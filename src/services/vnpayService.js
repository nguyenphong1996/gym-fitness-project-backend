...
exports.updateTransactionStatus = async ({ txnRef, rspCode, transactionStatus, params, source = 'return' }) => {
    ...
    // Lưu token nếu có trong params và giao dịch thành công
        ...
        if (tx.packageId && tx.userId) {
            ...
            try {
                if (tx.isRenewal) {
                    // Renewal: extend endDate for same package using activateMembership
                    await membershipService.activateMembership(tx.userId, resolvedPackageId, tx.txnRef, tx.billingCycle);
                    logInfo('vnpayService.updateTransactionStatus', 'Auto-renewed membership', {
                        userId: tx.userId,
                        packageId: resolvedPackageId,
                        txnRef: tx.txnRef,
                        billingCycle: tx.billingCycle
                    });
                } else if (tx.isUpgrade && tx.isTemporary) {
